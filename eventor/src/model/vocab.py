...
class Vocab(object):
    ...
    def getIndices(self, key, default=None):
    ...
    def convertToIdx(self, labels, unkWord, bosWord=None, eosWord=None):
        ...
        if bosWord is not None:
            vec += [self.getIndices(bosWord)]

        unk = self.getIndices(unkWord)
        vec += [self.getIndices(label, default=unk) for label in labels]

        if eosWord is not None:
            vec += [self.getIndices(eosWord)]
